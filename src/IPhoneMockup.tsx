...
const DEVICE_SPECS: Record<IPhoneModel, { w: number; h: number; radius: number; bezel: number; topSafe: number; bottomSafe: number; notch?: { w: number; h: number; r: number }; island?: { w: number; h: number; r: number } }> = {
...
function shade(hex: string, pct: number): string {
  ...
  if (!m) return hex
  const k = (100 + pct) / 100
  const to = (v: number) => Math.max(0, Math.min(255, Math.round(parseInt(v.toString(16).padStart(2,'0'), 16) * k)))
...
export const IPhoneMockup: React.FC<IPhoneMockupProps> = ({
  ...
  const insets = { top: safeAreaOverrides?.top ?? spec.topSafe, bottom: safeAreaOverrides?.bottom ?? spec.bottomSafe, left: safeAreaOverrides?.left ?? 0, right: safeAreaOverrides?.right ?? 0 }
  const cutout: CSSProperties = { position: 'absolute', left: '50%', transform: 'translateX(-50%)', background: '#000', zIndex: 2, boxShadow: '0 1px 2px rgba(0,0,0,0.7)' }
  ...
  return (
    <div className={className} style={{ boxSizing: 'border-box', display: 'inline-block', transform: `scale(${scale})`, transformOrigin: 'top left', ...style }}>
      <div style={{ width: outerWidth, height: outerHeight, borderRadius: resolvedRadius + resolvedBezel, background: frameGradient, padding: resolvedBezel, boxSizing: 'border-box', boxShadow: outerShadow, position: 'relative', overflow: 'hidden', ...frameStyle }}>
        <div style={{ width: '100%', height: '100%', borderRadius: resolvedRadius, position: 'relative', overflow: 'hidden', background: screenBg, boxShadow: innerShadowCss, ...screenStyle }}>
          {wallpaper && <div style={{ position: 'absolute', inset: 0, backgroundImage: `url(${wallpaper})`, backgroundSize: wallpaperFit, backgroundPosition: wallpaperPosition, backgroundRepeat: 'no-repeat', zIndex: 0 }} />}
          {useIsland && finalIslandW > 0 && <div style={{ ...cutout, top: 12, width: finalIslandW, height: finalIslandH, borderRadius: finalIslandR }} />}
          {!useIsland && useNotch && finalNotchW > 0 && <div style={{ ...cutout, top: 8, width: finalNotchW, height: finalNotchH, borderRadius: finalNotchR }} />}
          <div style={contentStyle}>{children}</div>
          {showHomeIndicator && <div style={{ position: 'absolute', bottom: 8, left: '50%', transform: 'translateX(-50%)', width: Math.round(screenWidth * 0.34), maxWidth: 140, height: 5, borderRadius: 3, background: 'linear-gradient(180deg, rgba(255,255,255,0.7), rgba(255,255,255,0.35))', opacity: 0.9, zIndex: 3 }} />}
        </div>
      </div>
    </div>
  )
